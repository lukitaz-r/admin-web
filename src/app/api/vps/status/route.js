...
export async function GET(request) {
  ...
  try {
    const res = await fetch(`${botApiUrl}/api/vps/status`, {
      headers: {
    ...
    console.error('[API Proxy VPS Status GET Exception]:', err);
    // If the server cannot be reached, return online: false gracefully so status badge handles it
    return NextResponse.json({ online: false, error: 'Failed to contact bot server' });
  }
}
